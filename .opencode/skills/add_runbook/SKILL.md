---
name: add_runbook
description: Use when a procedure was explained ad-hoc for the second time — converts it into a runbook with verification and rollback. Org-level procedures go to 02-organization/runbooks/, work-unit-specific ones to the work unit folder.
---
...
1. Decide the home: applies across the org →
   `02-organization/runbooks/`; specific to one work unit →
   `03-work/<wu>/`. Copy the runbook template; kebab-case action name.
...
5. Index it (runbooks README or work-unit overview). `make validate`.
