...
.PHONY: help init validate workspace stats

help:  ## Show available commands
	@grep -E '^[a-zA-Z_-]+:.*?## .*$$' $(MAKEFILE_LIST) | awk 'BEGIN {FS = ":.*?## "}; {printf "  \033[36m%-12s\033[0m %s\n", $$1, $$2}'

init:  ## Instantiate: make init ORG="Acme Inc." [PROFILE=consulting|delivery-oversight|development|full]
...
validate:  ## Structure (per config), links, duplicate IDs, unsourced decisions, status debt
